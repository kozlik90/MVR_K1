...
MainTask::MainTask(int n_, int m_, double eps_, double omega_, unsigned int Nmax_) : n(n_), m(m_), n2(2*n_), m2(2*m_), v1(n+1, std::vector<double>(m+1, 0)),
v2(n2+1, std::vector<double>(m2 + 1, 0)), Error(n+1, std::vector<double>(m+1, 0)), eps(eps_), eps2(eps_/10.0), omega(omega_), 
omega2(omega_), Nmax(Nmax_), Nmax2(Nmax_ * 1.5), iter(0), iter2(0), maxDiff(0.0), maxDiff2(0.0), nevyazkaMax(0.0), nevyazkaMax2(0.0)
 {
    if(omega >= 2) {
        double hx = 1.0 / n;
        double hx2 = 1.0 / n2;
...
void MainTask::calculateChisl(std::vector<std::vector<double>>& v, int n_, int m_, double eps_, double omega_, unsigned int Nmax_, int& iter_, double& maxDiff_) {
    double hx = 1.0 / n_;
    double ky = 1.0 / m_;
...
void MainTask::calculateError() {
	...
	std::cout << "File 3_pogreshnost is not open.\n";
	//return 1;
    }
    outFile33 << ";";
    for (int i = 0; i <= n; i++) {
    ...
    outFile33.close();
    std::cout << "File 3_pogreshnost.csv is ready" << std::endl;
    for (int i = 1; i < n; i++) {
        for (int j = 1; j < m; j++) {
            double x = i * hx2;
            double y = j * ky2;
...
void MainTask::printInfo() {
    ...
    std::cout << "A half-step grid is used to control the accuracy, SOR with the parameter omega2 = " << omega2 << ", criteria for stopping by accuracy eps2 = " << eps2 << " and by the number of iterations Nmax2 " << Nmax2 << "\n";
    std::cout << "iterations spent  N2 = " << iter2 << " the achieved accuracy of the iterative method eps^(N2) = " << maxDiff2 << "\n";
    std::cout << "The discrepancy ||R^(N2)|| = " << nevyazkaMax2 << "\n";
    std::cout << "The main task should be solved with an error of no more than eps = 0.5*10^(-6); the task was solved with an error of eps2 = " << maxError << "\n";
